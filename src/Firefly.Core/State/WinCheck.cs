...
namespace Firefly.Core.State
{
    ...
    /// <summary>
    /// Story Card victory (GF9 "Winning the Game"): the first player who meets
    /// the printed win condition wins immediately. Last-call / most-credits
    /// stories are evaluated only at EndOfTurn after the table has settled.
    /// </summary>
    public static class WinCheck
    {
        ...
        public static bool TryCompleteGoal(GameState game, PlayerState player, int number, out string? error)
        {
            ...
            if (goal == null)
            {
                error = $"Story Card has no Goal {number}.";
                return false;
            }
            if (player.CompletedGoals.Contains(number))
            {
                error = $"Goal {number} is already complete.";
                return false;
            }
        ...
        public static WinResult? Evaluate(GameState game, WinPhase phase)
        {
            ...
            switch (scenario.WinType)
            {
                case "firstToCompleteGoal":
                    return FirstWhere(game, p => p.CompletedGoals.Contains(scenario.WinGoal),
                        $"completed Goal {scenario.WinGoal}");
                ...
                case "endTurnWithCash":
                    if (phase != WinPhase.EndOfTurn)
                        return null;
                    return FirstWhere(game, p => p.Cash >= scenario.WinCash,
                        $"ended a turn with ${scenario.WinCash}");
                case "firstWithGoalTokens":
                    return FirstWhere(game, p => p.GoalTokens >= scenario.WinGoalTokens,
                        $"collected {scenario.WinGoalTokens} Goal tokens");
                case "firstWithGoalTokensAtStartOfTurn":
                    if (phase != WinPhase.StartOfTurn)
                        return null;
                ...
                case "deferredConsensus":
                case "selectedSoloGoalBeforeTimeExpires":
                    return null;
                default:
                    return null;
            }
        ...
        public static bool TryFinishTravelPay(GameState game, string playerId, out string? error)
        {
            ...
            if (goal == null)
            {
                error = "This Story Card has no travel/pay Goal.";
                return false;
            }
            if (!AtLocation(game.Map, player.SectorId, goal.Location))
            {
                error = $"Must be at {goal.Location}.";
                return false;
            }
            if (player.GoalTokens < goal.GoalTokens)
            {
                error = $"Need {goal.GoalTokens} Goal token(s).";
                return false;
            }
            if (player.Cash < goal.Pay)
            {
                error = $"Need ${goal.Pay}.";
                return false;
            }
        ...
        private static bool AutoMet(GameState game, PlayerState player, ScenarioGoal goal)
        {
            switch (goal.Type)
            {
                case "solidWithDistinctContacts":
                    return ActiveAlertRules.CountedSolidCount(game, player) >= Math.Max(1, goal.Count);
                case "cashAndGoalToken":
                    return player.Cash >= goal.Cash && player.GoalTokens >= Math.Max(1, goal.GoalTokens);
                case "cashGrantsGoalToken":
                    return player.Cash >= goal.Cash;
                case "solidWithContacts":
                    return IsSolidWithAll(game, player, goal);
                default:
                    return false;
            }
        }
        ...
        private static bool AtHaven(PlayerState player) =>
            ...
            && string.Equals(player.SectorId, player.HavenSectorId, StringComparison.Ordinal);

        private static bool AtLocation(SectorMap map, string sectorId, string? location)
        {
            if (string.IsNullOrWhiteSpace(location))
                return true;
            return map.SatisfiesDestination(sectorId, location);
        }
        ...
        private static WinResult? FirstWhere(GameState game, Func<PlayerState, bool> pred, string reason)
        {
            ...
            {
                if (pred(player))
                    return new WinResult(player.Id, player.Name, reason);
            }
            return null;
        }
        ...
        private static WinResult? Richest(GameState game, string reason)
        {
            ...
            foreach (var player in game.Players)
            {
                if (best == null || player.Cash > best.Cash)
                    best = player;
            }
